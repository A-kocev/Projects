...
@section('content')
<form action="{{route('discounts.update', $discount)}}" method="POST" class="flex flex-col justify-between min-h-screen py-5">
    @csrf
    @method('PUT')
    <div>
        <div class="flex justify-between items-center">
            ...
            <select name="status" id="discountStatus" class="border-gray-300 focus:border-rose-500 focus:ring-rose-500 rounded-md shadow-sm text-sm pr-5 pl-1">
                ...
                <option value="0" {{ $discount->status == 0 ? 'selected' : '' }}>Архивирано</option>
                <option value="1" {{ $discount->status == 1 ? 'selected' : '' }}>Активно</option>
            </select>
        </div>
        <!-- name -->
        <div class="mt-8">
            <x-input-label for="name" :value="__('Име на попуст')" />
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" value="{{$discount->name}}" required />
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>
        <!-- percentage -->
        <div class="mt-4">
            <x-input-label for="percentage" :value="__('Попуст')" />
            <x-text-input id="percentage" class="block mt-1 w-full" type="number" min="1" max="99.99" step="0.1" name="percentage" value="{{$discount->percentage}}" required />
        ...
        <div class="mt-4">
            ...
            <select name="discount_products[]" id="discount_products" multiple>
                ...
                <option value="{{$product->id}}" {{$product->discount_id == $discount->id ? 'selected' : ''}}>{{$product->id}}-{{$product->name}}</option>
                @endforeach
            </select>
        </div>
    </div>
